...
internal static class CommandLine
{
    ...
    private static void PrintUsage() =>
        Console.WriteLine("""
            ...
            Environment:
              CI_REPO_ROOT       Repository root (default: current directory)
              CI_MAIN_STORE      main's artifact volume (default: artifacts/ci-main)
              CI_PR_STORE        This run's own artifact volume (default: artifacts/ci-pr)
              CI_CACHELESS       true/1 to ignore main and rebuild everything (PR runs only;
                                 never promotes)
              CI_COVERAGE_HTML   0 to skip the HTML coverage report (default: on locally, off in CI)
            """);
}
