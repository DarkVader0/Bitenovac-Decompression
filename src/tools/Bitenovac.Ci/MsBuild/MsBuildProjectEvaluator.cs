...
internal sealed class MsBuildProjectEvaluator : IDisposable
{
    ...
    private EvaluatedProject Evaluate(string relativePath, string configuration)
    {
        ...
        foreach (var item in project.GetItems("CiHashInput"))
    ...
    // Only files inside the repository are hashed individually: everything the SDK itself
    // imports lives outside the repository root and changes only when NETCoreSdkVersion does,
    // which is already an own-hash input above. Hashing every SDK .targets file on top of that
    // would add nothing but a slow, redundant filesystem walk per project.
    //
    // Two kinds of repository import are deliberately skipped, because hashing them whole is
    // strictly worse than the resolved package closure that already covers what they mean:
    //
    //   Directory.Packages.props  Central package management imports it into every project, so
    //                             hashing its content would put every package version in the
    //                             repository into every project's own hash — bumping a package
    //                             one project uses would invalidate all of them, which is the
    //                             exact over-building this cache exists to stop. What a bump
    //                             actually does to a given project shows up, precisely and only
    //                             where it applies, in that project's project.assets.json.
    //
    //   obj/*.nuget.g.props       Generated by restore, and it embeds the absolute path of the
    //   obj/*.nuget.g.targets     NuGet package folder. That path differs between this machine
    //                             and a CI container (NUGET_PACKAGES=/cache/nuget), so hashing
    //                             it would guarantee a miss for every project every time the
    //                             cache moved between them, for a difference that changes no
    //                             compiled output. Its real content is the resolved closure.
    ...
    // .editorconfig cascades — every level from the project's own directory up to the
    // repository root applies, unlike Directory.Build.props/targets where only the nearest one
    // wins. Each one found is hashed independently so a change at any level invalidates.
