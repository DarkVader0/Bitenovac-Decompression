...
internal static class TestFactory
{
    ...
    public static DiveProfile CreateProfile(params (double DepthMeter, double Minutes)[] levels) => new DiveProfile(levels.Select(static level => new DiveSegment(Depth.FromMeter(level.DepthMeter), TimeSpan.FromMinutes(level.Minutes), GasMixture.Air, SegmentKind.Bottom)));
    ...
    public static DiveProfile CreateProfile(BreathingLoop loop,
        params (double DepthMeter, double Minutes)[] levels) => new DiveProfile(levels.Select(level => new DiveSegment(Depth.FromMeter(level.DepthMeter),
    ...
    public static DiveProfile CreateProfile(
        params (double DepthMeter, double Minutes, BreathingLoop Loop)[] levels) => new DiveProfile(levels.Select(static level => new DiveSegment(Depth.FromMeter(level.DepthMeter),
